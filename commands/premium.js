        ...
        if (args[1] === "redeem") {
            ...
            if (settings.tokens === 1) {
                const premiumEnabled = new Discord.MessageEmbed()
                    ...
                    .setDescription(`<:green_check:826893895504887828> Premium has been enabled for **${message.guild.name}**`)
            ...
            if (settings.guildID === message.guild.id) {
                const disabledPremium = new Discord.MessageEmbed()
                    .setTitle("Premium")
                    .setColor("ff5959")
                    .setDescription(`<:red_check:829200653796245504> Premium was disabled on **${message.guild.name}** and \`1\` premium token has been added to your balence`)
                    .setFooter(`${message.author.tag} - ${message.guild.name}`)
            ...
            if (settings.guildID === message.guild.id) message.channel.send(yesPremium);
            if (settings.tokens === 1) message.channel.send(yesTokens)
            if (cPrem) {
                const otherServer = new Discord.MessageEmbed()
                    ...
                    .setFooter(`${message.author.tag} - ${message.guild.name}`)
                message.channel.send(otherServer)
            } else {
                const otherServer = new Discord.MessageEmbed()
                    .setTitle("Premium")
                    .setColor(guildSettings.color)
